...
echo "--START--"
echo "--STEP (1 / 4)-- Creating directory for docker [~/.jsxopensuse]"
rm -rf ~/.jsxopensuse
mkdir ~/.jsxopensuse
echo "--DONE--"
echo "--STEP (2 / 4)-- Moving all utils into directory [Dockerfile, runner.sh, OpenSuse.Tumbleweed] => [~/.jsxopensuse]"
cp ./Dockerfile ./runner.sh ./jsxos_packages ./packages -t ~/.jsxopensuse
echo "--DONE--"
echo "--STEP (3 / 4)-- Building docker image on this computer as \"jsxopensuse\""
...
echo "--DONE--"
echo "--STEP (4 / 4)-- Move executables running and update \"jsxopensuse\""
